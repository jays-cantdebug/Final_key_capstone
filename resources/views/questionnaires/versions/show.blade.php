...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <h2 class="text-2xl font-semibold text-body dark:text-slate-100">Version v{{ $version->version_number }}</h2>
            </div>
    ...
    <x-card class="mb-6">
        <div class="flex items-start justify-between gap-4">
            <dl class="grid gap-4 sm:grid-cols-2">
                <div>
                    <dt class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Effective Date</dt>
                    <dd class="mt-1 text-sm font-medium text-body dark:text-slate-100">{{ $version->effective_date->format('M d, Y') }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">Questions</dt>
                    <dd class="mt-1 text-sm font-medium text-body dark:text-slate-100">{{ $questions->count() }}</dd>
                </div>
            </dl>
    ...
    <x-table>
        <x-slot:header>
            <h3 class="text-lg font-semibold text-body dark:text-slate-100">Questions</h3>
            <div class="flex flex-wrap gap-2">
                @if ($version->isEditable())
                ...
                @elseif ($version->status === 'Active')
                    <form id="archive-version-form" method="POST" action="{{ route('questionnaires.versions.archive', [$questionnaire, $version]) }}" class="hidden">
                        @csrf
                        @method('PATCH')
                    </form>
                    <button
